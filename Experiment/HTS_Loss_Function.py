...
class MSE_Smoothness_Loss(nn.Module):
    ...
    def forward(self, pred):
        smooth_error = self.conv1d(pred)**2 # 预测值前后相减的平方，那也就是我希望前后值预测的结果差别越小越好
...
class Weighted_MSE_Loss(nn.Module):
    ...
    def forward(self, pred, target):
        se  = (pred-target)**2                    #  也就是希望末尾的预测结果一定要准
        out = se * self.weights.expand_as(target) #  越靠后的预测值的损失，我们基于的权重就越大，给予的惩罚也就越大
        loss = out.mean() 
        return loss
