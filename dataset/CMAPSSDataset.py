...
class CMAPSSDataset(Dataset):
    ...
    @staticmethod
    def get_datasets(dataset_root, sub_dataset,di_root=None, sequence_len=1, max_rul=None,
        ...
        if sub_dataset == 'PHM08':
            ...
            train_df = pd.read_csv(os.path.join(dataset_root, 'train_{:s}.txt'.format(sub_dataset)), sep=' ', header=None)
            ...
            train_di = test_di = []
            if di_root is not None:
                # Assuming degradation_file is a CSV with columns ['id', 'time', 'degradation']
                print('di_root', di_root)
                colnames = ['id', 'time', 'DI']
                train_di = pd.read_csv(os.path.join(di_root, 'train_DI_{:s}.csv'.format(sub_dataset)), sep=',',names=colnames, header=None)
                test_di = pd.read_csv(os.path.join(di_root, 'test_DI_{:s}.csv'.format(sub_dataset)), sep=',',names=colnames, header=None)
                print('train_di.shape', train_di.shape, 'test_di.shape', test_di.shape)
        ...
        return train_dataset, test_dataset, valid_dataset, train_di, test_di
    ...
    def get_data_loaders(loader_kwargs, train_kwargs=None, test_kwargs=None, valid_kwargs=None, **dataset_kwargs):
        ...
        train_dataset, test_dataset, valid_dataset, train_di, test_di = CMAPSSDataset.get_datasets(**dataset_kwargs)
    ...
    def count_rul(self):
        ...
        # = lambda x: x-30 if x > self.max_rul else (x+30 if x < self.max_rul-30 else x)
        #max_rul = np.clip(int(df.mean().time), self.max_rul-15, self.max_rul)
