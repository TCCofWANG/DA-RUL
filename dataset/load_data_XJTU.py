...
# 计算训练集和测试集的合力vibration, 再将训练集根据文件分为两个
    ...
    train_y_array_simple = np.copy(train_y_array)

    for i in range(len(train_y_array_simple)):
        added_split_train_y = np.linspace(0, 1, int(32768 / split_ratio) - 1)
        train_y_array = np.insert(train_y_array, (len(added_split_train_y) + 1) * i + 1,
                                  added_split_train_y[::-1] + train_y_array_simple[i])  # TODO 为什么全部自动变成整数？
    ...
    test_y_array_simple = np.copy(test_y_array)
    for i in range(len(test_y_array_simple)):
        added_split_test_y = np.linspace(0, 1, int(32768 / split_ratio) - 1)
        test_y_array = np.insert(test_y_array, (len(added_split_test_y) + 1) * i + 1,
                                 added_split_test_y[::-1] + test_y_array_simple[i])  # TODO 为什么全部自动变成整数？
    ...
    max_life = max(np.max(y_train).tolist(), np.max(test_y).tolist())
    ...
    return train_X, train_y, vali_X, vali_y, test_X, test_y, max_life
