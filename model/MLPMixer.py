...
class DualMLPMixer(nn.Module):
    def __init__(self, window_size=30, in_features=16, hidden_dim=32,num_layers=6,dropout=0,
                 model_flag="MLPDualMixer-h32-6", device="cuda:0", label_norm=True,
                 filter_size=0):
        super().__init__()
    ...
    def forward(self, x, label=None):
        if len(x.shape) < 4:
            ...
            f1 = self.out_gat1(f1.transpose(-1, -2))
            f2 = self.out_gat2(f2)
            x = torch.flatten(f1.transpose(-1, -2) + f2, start_dim=-2, end_dim=-1)
            return self.output(x)
        else:
            f_pos, f_apos, f_neg, weight = self.generate_contrastive_samples(x, label)
            return pn_rul_compute(self.output, f_pos, f_neg), f_pos, f_apos, f_neg, weight
