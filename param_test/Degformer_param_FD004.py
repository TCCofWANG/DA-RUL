...
def train_model_with_hyperparameters(args, model_kwargs):
    #seeds = [8, 16, 24, 32,40, 42]
    seed = 42#random.choice(seeds)
...
def main(args):
    ...
    for combination in product(*hyperparameters_grid.values()):
        ...
        model_kwargs = {
            ...
            'hidden_dim':args.hidden_dim, #((args.sequence_len*2) + 7) // 8 * 8,#rounding sequence_len up to the nearest multiple of 8
...
if __name__ == '__main__':
    ...
    parser.add_argument('--cell-type', type=str, default='slstm', help='slstm, lstm, gru or rnn')  # done
